...
rem link mpmissions folder
if not exist %SERVERDIR%\mpmissions (
    mkdir %SERVERDIR%\mpmissions
)
if exist %SERVERDIR%\deploy\mpmissions (
    rmdir /q %SERVERDIR%\deploy\mpmissions
)
mklink /J %SERVERDIR%\deploy\mpmissions %SERVERDIR%\mpmissions

rem set up the command to start the server
IF DEFINED FORCE32 (
  SET cmd=\"%ARMAPATH%\arma3server.exe\" -profiles=%SERVERDIR%\profiles -config=%CONFIGFILE%
) ELSE (
  SET cmd=\"%ARMAPATH%\arma3server_x64.exe\" -profiles=%SERVERDIR%\profiles -config=%CONFIGFILE%
)
